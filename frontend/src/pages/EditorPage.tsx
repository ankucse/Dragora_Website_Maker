...
export default function EditorPage() {
  ...
  const setIsPreview = useEditorStore(s => s.setIsPreview);
  ...
  useEffect(() => {
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        ...
        if (selectedId) {
          e.preventDefault();
          const removeComponent = useEditorStore.getState().removeComponent;
  ...
  const formattedProjectName = id === 'new' 
    ? 'Untitled Project' 
    : (id === 'unicorn-saas' ? 'Unicorn SaaS Landing Page' : id?.replace(/[-_]/g, ' ').replace(/\b\w/g, c => c.toUpperCase()));
      ...
      <div className="h-screen w-screen bg-[#0a0a0a] flex flex-col overflow-hidden font-sans text-zinc-300">
        ...
        {!isPreview && (
          ...
          >
            ...
            <div className="flex items-center gap-4 text-sm font-medium">
              <span className="text-white text-xs font-semibold hidden sm:inline-block bg-white/5 px-3 py-1 rounded-md border border-white/5">{formattedProjectName}</span>
        ...
        {isPreview && (
          <button 
            ...
            className="fixed bottom-6 right-6 z-50 bg-indigo-600 hover:bg-indigo-500 text-white px-5 py-2.5 rounded-full text-sm font-bold tracking-wide transition-all shadow-[0_10px_30px_rgba(79,70,229,0.5)] flex items-center gap-2 border border-indigo-400/20"
          >
            <Monitor className="w-4 h-4" /> Exit Preview
          </button>
        )}
        ...
        <div className="flex-1 flex relative">
          ...
          <div className="flex-1 relative flex justify-center overflow-hidden transition-all duration-300">
             ...
             <div className="h-full border-x border-white/5 bg-[#0a0a0a] transition-all duration-500 flex flex-col" style={{ width: (isPreview || deviceMode === 'desktop') ? '100%' : deviceMode === 'tablet' ? '768px' : '375px' }}>
                <Canvas />
             </div>
          </div>
